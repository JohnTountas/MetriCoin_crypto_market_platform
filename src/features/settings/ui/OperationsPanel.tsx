...
import { Badge, Button, Card, formatTimestamp,Input, SectionHeading, Skeleton } from '@/shared';
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div className="surface-subtle rounded-2xl p-4">
            <div className="flex items-center justify-between">
              ...
              <Badge tone={statusToneMap[isServerConnected ? 'connected' : 'disconnected']}>
            ...
            <p className="mt-1 text-sm text-[var(--text-muted)]">Live alerts owned by the server poller</p>
          </div>
          ...
          <div className="surface-subtle rounded-2xl p-4">
            ...
            <p className="mt-1 text-sm text-[var(--text-muted)]">Persisted alert deliveries and monitor events</p>
          </div>
          ...
          <div className="surface-subtle rounded-2xl p-4">
            ...
            <p className="mt-1 text-sm text-[var(--text-muted)]">Tracked product events in the last retained window</p>
          </div>
          ...
          <div className="surface-subtle rounded-2xl p-4">
            ...
            <p className="mt-1 text-sm text-[var(--text-muted)]">Unhandled browser and async failures captured</p>
          </div>
        </div>

        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_380px]">
          <div className="space-y-4">
            <div className="surface-subtle rounded-2xl p-4">
              <div className="flex items-center gap-2">
                <Siren className="h-4.5 w-4.5 text-[var(--highlight-text)]" />
                <p className="text-sm font-semibold text-[var(--text-primary)]">Recent server notifications</p>
              </div>
              <div className="mt-4 space-y-3">
                {notificationsQuery.isLoading ? <Skeleton className="h-28 rounded-2xl" /> : null}
                {!notificationsQuery.isLoading && (notificationsQuery.data ?? []).length === 0 ? (
                  <p className="text-sm leading-6 text-[var(--text-muted)]">
                    No server notifications yet. Sync the app with the ops server and let an alert cross a live price threshold.
                  </p>
                ) : null}
                {(notificationsQuery.data ?? []).slice(0, 6).map((notification) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <Badge tone={notification.webhookStatus === 'failed' ? 'warning' : 'positive'}>
                        {notification.webhookStatus}
                      </Badge>
                    </div>
                    <p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">{notification.description}</p>
            ...
            <div className="surface-subtle rounded-2xl p-4">
              <div className="flex items-center gap-2">
                <Bug className="h-4.5 w-4.5 text-[var(--negative-text)]" />
                <p className="text-sm font-semibold text-[var(--text-primary)]">Recent captured errors</p>
              </div>
              <div className="mt-4 space-y-3">
                {telemetryQuery.isLoading ? <Skeleton className="h-24 rounded-2xl" /> : null}
                {!telemetryQuery.isLoading && (telemetryQuery.data?.recentErrors.length ?? 0) === 0 ? (
                  <p className="text-sm leading-6 text-[var(--text-muted)]">
                    No recent client-side errors captured by the telemetry pipeline.
                  </p>
                ) : null}
                {(telemetryQuery.data?.recentErrors ?? []).map((event) => (
                  ...
                  >
                    ...
                    <p className="mt-2 text-xs uppercase tracking-[0.18em] text-[var(--text-faint)]">
                      {event.route ?? 'unknown route'} | {formatTimestamp(event.createdAt)}
                    </p>
                  </div>
                ))}
              </div>
          ...
          <Card className="surface p-5">
            ...
            <form
              ...
              onSubmit={form.handleSubmit(async (values) => {
                ...
                const normalizedValues = {
                  ...values,
                  webhookUrl: trimmedWebhookUrl === '' ? undefined : trimmedWebhookUrl,
                };
                ...
                try {
                  ...
                  pushToast({
                    ...
                    description: 'The ops server will use the updated delivery configuration on the next poll cycle.',
                  });
                  ...
                  pushToast({
                    tone: 'error',
                    title: 'Save failed',
                    description: 'Metricoin could not update the ops server notification settings.',
                  });
                }
              })}
            >
              <label className="flex items-start gap-3 rounded-2xl border border-[var(--border)] px-4 py-3">
                ...
                />
                <div>
                  <p className="text-sm font-semibold text-[var(--text-primary)]">Store in-app notification feed</p>
                  <p className="mt-1 text-sm leading-6 text-[var(--text-muted)]">
                    Keep a server-owned delivery inbox that the UI can poll and render.
                  </p>
                </div>
              </label>
              ...
              <label className="flex items-start gap-3 rounded-2xl border border-[var(--border)] px-4 py-3">
                ...
                />
                <div>
                  <p className="text-sm font-semibold text-[var(--text-primary)]">Enable server-side alert polling</p>
                  <p className="mt-1 text-sm leading-6 text-[var(--text-muted)]">
                    Keep monitoring synced alerts even when the browser is no longer on the watchlist page.
                  </p>
                </div>
              </label>
              ...
              <label className="space-y-2 text-sm text-[var(--text-secondary)]">
                ...
                <p className="text-xs leading-6 text-[var(--text-faint)]">
                  Optional. When set, the ops server will POST alert deliveries to this endpoint.
                </p>
                {form.formState.errors.webhookUrl?.message ? (
                  <p className="text-xs text-[var(--negative-text)]">{form.formState.errors.webhookUrl.message}</p>
                ) : null}
              </label>
            ...
            <div className="surface-subtle mt-6 rounded-2xl p-4 text-sm text-[var(--text-secondary)]">
              <div className="flex items-center gap-2">
                ...
                <p className="font-semibold text-[var(--text-primary)]">Current server snapshot</p>
              </div>
              <p className="mt-3">Polling enabled: {notificationSettings.alertPollingEnabled ? 'yes' : 'no'}</p>
              <p className="mt-1">In-app feed: {notificationSettings.enableInApp ? 'enabled' : 'disabled'}</p>
              <p className="mt-1">Webhook configured: {notificationSettings.webhookUrl ? 'yes' : 'no'}</p>
            </div>
          </Card>
        </div>
      </Card>
